...
def call(Map config) {
// create a custom build url
    def customBuildUrl = "${config.customJenkinsUrl}/job/${config.customJobName}/${env.BUILD_NUMBER}"
    script {
        def customMailRecipients = "${config.customMailRecipients}"
        def defaultMailRecipients = '$DEFAULT_RECIPIENTS'
        def mailRecipients = "${customMailRecipients} ${defaultMailRecipients}"
        def emailSubject = "Project # ${currentBuild.fullProjectName} - Job # ${currentBuild.projectName} - Build # [${env.BUILD_NUMBER}] - ${currentBuild.currentResult}"
        emailext (
                subject: emailSubject,
                body: "Follow the link ${customBuildUrl} to view the job results",
// to: mailRecipients, DEFAULT_RECIPIENTS
                to: mailRecipients
        )
    }
}