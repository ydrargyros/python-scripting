...
def call(String targetEnv, Map config) {
    ...
    container('helm') {
        withCredentials([file(credentialsId: envConfig.credsId, variable: 'KUBECONFIG')]) {
            sh "helm ls"
            sh "helm upgrade ${chartProject} --namespace ${envConfig.namespace} --install charts --atomic -f charts/envs/${targetEnv}/values.yaml --set images.dockerRegistry=${config.build.dockerRegistry.harborUrl} --debug"
        } // withCredentials
    }
