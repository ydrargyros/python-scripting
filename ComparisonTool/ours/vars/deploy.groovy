...
def call(Map config, String deployType) {
    ...
    if (params.containsKey("SKIP_STAGE1_DEPLOY") && params.SKIP_STAGE1_DEPLOY == false) {
        "$deployType"("stage1", config)
    }
    ...
    if (params.containsKey("SKIP_AZDEMO_DEPLOY") && params.SKIP_AZDEMO_DEPLOY == false) {
        "$deployType"("azdemo", config)
    }
